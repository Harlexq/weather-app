...
export class AuthComponent {
  ...
  getUsers() {
    this.http.get<User[]>('adminUsers', (res) => {
      this.users = res;
    });
  }
  ...
  signIn() {
    this.users.find((user) => {
      if (user.email === this.signInEmail.value) {
        this.authService.login('adminLogin', this.signInForm.value, (res) => {});
      } else {
  ...
  signUp() {
    ...
    if (userExists) {
      ...
      this.authService.signup('adminSignup', this.signUpForm.value, (res) => {
        this.errorMessage = '';
      });
    }
  }
